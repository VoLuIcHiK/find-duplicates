...
from .rabbit.ысруьфы import NNOutput
...
class RabbitConsumerThread(Thread):
    ...
    def filter_call(self, ch, method, body):
        ...
        try:
            ...
            nn_output: NNOutput = NNOutput.from_dict(body_json)
            logger.info(f"Parsed task NNOutput: {nn_output.model_dump()}")
            task_id = nn_output.inputs.task_id
            failed = False
            ...
            with self._lock:
                ...
                if task is None:
                    ...
                    return
                task.result = nn_output.result.model_dump()
