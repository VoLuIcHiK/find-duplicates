...
def compact_publish_data(rabbit_url: str, output_queue: str, message: str):
    con = Connector(rabbit_url)
    with con as (connection, channel):
        connection: Connection
        channel: Channel
        con.create_queue(channel, output_queue)
        channel.basic_publish(body=message.encode('utf-8'), exchange='', routing_key=output_queue)


def compact_get_data(rabbit_url: str, input_queue, block=False) -> str:
    con = Connector(rabbit_url)
    with con as (connection, channel):
        connection: Connection
        channel: BlockingChannel
        con.create_queue(channel, input_queue)
        channel.basic_qos(prefetch_count=1)
        while True:
            x = channel.basic_get(input_queue)
            if x != (None, None, None):
                channel.basic_ack(x[0].delivery_tag)
                message_bytes = bytes(x[2])
...
class Connector:
    ...
    def create_queue(channel, queue_name):
        ...
        logger.info(f'Queue \"{queue_name}\" has been created')
