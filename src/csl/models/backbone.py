""" 
    ...
    # Code is modified from https://github.com/facebookresearch/TimeSformer
"""
import numpy as np
import torch
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # print("block attention type: {}".format(self.attention_type))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224,  patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.fc_norm = norm_layer(embed_dim) if use_mean_pool else None
...
class Timesformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, num_classes=1, num_frames=8, vit_type="base",
                        attention_type='divided_space_time',  in_channels=3,
                        use_mean_pool=False,
                        **kwargs):
        super(Timesformer, self).__init__()
     
        assert vit_type in ['base', 'small']
        if vit_type == "base":
