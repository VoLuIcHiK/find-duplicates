...
class Preprocess():
    ...
    def __call__(self, video_path):
        ...
        tensor_frames = [transform(image=frame)['image'] for frame in frames]
        frames = np.array(tensor_frames).reshape((-1, 8, 224, 224, 3))
        # tensor_frames = torch.Tensor(tensor_frames)
        # tensor_frames = (tensor_frames / 255.0 - 0.5) / 0.5
